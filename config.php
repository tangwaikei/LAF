...
//数据库连接
//$localPdo=new PDO("mysql:host=".SAE_MYSQL_HOST_M.";port=".SAE_MYSQL_PORT.";dbname=".SAE_MYSQL_DB.";charset=utf8",SAE_MYSQL_USER,SAE_MYSQL_PASS);
$localPdo=new PDO("mysql:host=localhost;dbname=root;charset=utf8",'root','localhost');

//页面访问控制
$fun=isset($_GET['fun'])? $_GET['fun']:(isset($_POST['fun'])? $_POST['fun']:'list');
$access=array("list"=>0,"details"=>0,"comment"=>1,"user"=>2,"add"=>2);
...
if(!isset($access[$fun])||$access[$fun]>$_SESSION['rank'])
{
    ...
    else $answer=array('code'=>-1,'msg'=>'请先登陆！','data'=>'');
